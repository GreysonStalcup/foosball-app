...
import { listenForScores, getRealtimeDatabase, getMostRecentScore } from '../firebase';
    ...
    <div className="grid h-screen place-items-center bg-gray-100">
      ...
      <div className="grid grid-cols-1 gap-4 w-1/2 text-center mx-auto">
        {Object.keys(scores).map((scoreId) => (
          <div key={scoreId} className="flex flex-row items-center justify-between">
            <div className="w-1/3">
              <h3 className="text-lg font-bold">{scores[scoreId].team1}</h3>
            </div>
            <div className="w-1/3 text-center">
              <p className="text-gray-800 text-3xl"><span className={`${scores[scoreId].team1Score > scores[scoreId].team2Score ? 'text-green-500' : 'text-gray-800'}`}>{scores[scoreId].team1Score}</span> - <span className={`${scores[scoreId].team1Score < scores[scoreId].team2Score ? 'text-green-500' : 'text-gray-800'}`}>{scores[scoreId].team2Score}</span></p>
            </div>
            <div className="w-1/3">
      ...
      <div className="flex justify-center my-8">
        ...
        <Link to="/auth/logout" className='bg-purple-600 transition ease-in-out text-white py-2 px-4 rounded-lg mr-4'>
          Sign Out
        </Link>
      </div>
      } 
